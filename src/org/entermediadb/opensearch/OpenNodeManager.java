...
public class OpenNodeManager extends BaseNodeManager implements Shutdownable {
	...
	public String restoreLatest(String inCatalogId, String lastrestored) {
		...
		restoreSnapShot(inCatalogId, info.toString());
...
//			{
//				AcknowledgedResponse delete = getClient().admin().indices().delete(new DeleteIndexRequest(currentindex)).actionGet();
//			}
//			ClusterHealthResponse health = admin.cluster().prepareHealth(indexid).setWaitForYellowStatus().execute().actionGet();
//		}
//		catch (Exception ex)
...
//	{
//
//		IndexAbstraction indexToAliasesMap = getClient().admin().cluster().state(Requests.clusterStateRequest()).actionGet().getState().getMetadata().getIndicesLookup().get(aliasName);
//
//		if (indexToAliasesMap == null)
//		{
//			return;
...
//			if (newindex != null)
//			{
//				AcknowledgedResponse delete = getClient().admin().indices().delete(new DeleteIndexRequest(newindex)).actionGet();
...
//						try
//						{
////							String out = JsonOutput.prettyPrint(source);
...
//			if (!ok)
//			{
//				AcknowledgedResponse delete = getClient().admin().indices().delete(new DeleteIndexRequest(tempindex)).actionGet();
//				if (!delete.isAcknowledged())
//				{
...
//		{
//
//			AcknowledgedResponse response = getClient().admin().indices().delete(new DeleteIndexRequest(oldindex)).actionGet();
